...
>
> *v3.5-e4 sync patch (July 2026): NAT v3.2-e1 deployment interface adopted in MM-13 — four-role memory topology (mediation/buffer map $\mathcal M_B$, independent reference layer $\mathcal M_R$), registry view separation with the no-flattened-latest-truth principle, and the star-topology ↔ memory-multiplex correspondence declaring MM-6's mediated-only rule and NAT §7.9.2.8 edge separation to be the same design at different granularity. Reciprocal: NAT v3.2-e1 adopts MM-4 transitions, MTRL-6, MM-8 e3 anchors, MM-16, B14–B16/B16-ADV, and the decision-procedure lock via NAT §13.1.1.*
>
...
**NAT v3.2 deployment interface (adopted v3.5-e4).** NAT §7.9.2 executes this architecture and supplies three objects RBIT adopts as canonical deployment structure rather than re-deriving:

1. **Four-role memory topology.** Beyond $\mathcal M_i$ and $\mathcal M_G$, a deployment SHOULD maintain an explicit **mediation/buffer map** $\mathcal M_B$ (unresolved translations, conflicts, candidate correspondences — the working store of CMCP-4) and an **independent reference layer** $\mathcal M_R$ (clean anchors, protected baselines, audit commitments — the home of $A_{\mathrm{clean}}$). These are logical roles; co-hosting is permitted only with explicit access control, update authority, provenance, and failure independence.
2. **Registry view separation.** The global memory SHOULD expose separate views for verified-common, agent-specific, conflict, unknown, stale, quarantined/revoked, lineage, and rollback-compatible historical terrain. A single flattened "latest truth" table is unsafe for heterogeneous agents because it hides scope, time, uncertainty, and local exceptions — flattening is itself a homogenization mechanism in the MM-12 sense.
3. **Topology correspondence.** The star topology declared in MM-6's topology note and NAT's memory-multiplex edge separation (task, evidence, patch-distribution, anchor-access, audit, quarantine edges need not coincide; NAT §7.9.2.8) are the same design stated at different granularity: "mediated-only peer exchange" is the collapsed form of "peer-relevant differentials travel on evidence and distribution edges through fusion, never on direct peer edges." Citing one implies the other; neither is a contradiction of the other. NAT's multiplex attack-surface mapping (NAT §7.9.2.11) shows each MM-16 attack targets a distinct edge type, making edge separation simultaneously the attack-surface partition and the containment mechanism.
